...
@section('content')
    ...
    <div class="container mt-3">
        <div class="overview-container">
            ...
            @endif
            @if (!$user?->employer?->isProfileCompleted())
                <div class="alert alert-info alert-dismissible fade show" role="alert">
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    <strong>Profile Setup!</strong> Your profile setup is not completed yet.
                </div>
            @endif
            @if (!$user?->employer?->isBusinessVerified())
                <div class="alert alert-secondary" role="alert">
                    <strong class="alert-heading fw-bold">Business Verification! </strong>
                    Verification Process Pending
                    <hr />
                    <p class="mb-0">Business not verified yet</p>
                </div>
            @endif
            <h5 class="h6 text-capitalize fw-semibold">
                hello, {{ $user->employer?->company_name ?? $user->username }}
            </h5>
                ...
                <div class="card mx-md-3">
                    <div class="card-body d-flex flex-row justify-content-between flex-nowrap">
                        <div class="d-flex flex-column justify-content-center">
                            <span class="fw-semibold fs-5 text-capitalize">my jobs (active)</span>
                        </div>
                        <div class="d-flex flex-row justify-content-center align-items-center">
                            <h6 class="fs-4 fw-semibold mb-0">{{ $user?->employer?->activeJobs->count() ?? 0 }}</h6>
                        </div>
                    </div>
                </div>
                <div class="card mx-md-3">
                    <div class="card-body d-flex flex-row justify-content-between flex-nowrap">
                        <div class="d-flex flex-column justify-content-center">
                            <span class="fw-semibold fs-5 text-capitalize">favourite candidates</span>
                        </div>
                        <div class="d-flex flex-row justify-content-center align-items-center">
                            <h6 class="fs-4 fw-semibold mb-0">{{ $user?->employer?->savedCandidates->count() ?? 0 }}</h6>
                        </div>
                    </div>
                </div>
                <div
                    class="card mx-md-3 {{ $user?->employer?->profileCompletion() == '100%' ? 'bg-secondary text-white rounded-start-2' : '' }}">
                    <div class="card-body d-flex flex-row justify-content-between flex-nowrap">
                        <div class="d-flex flex-column justify-content-center">
                            <span class="fw-semibold fs-5 text-capitalize">Profile Completion</span>
                        </div>
                        <div class="d-flex flex-row justify-content-center align-items-center">
                            <h6 class="fs-4 fw-semibold mb-0">{{ $user?->employer?->profileCompletion() ?? '0%' }}</h6>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table class="table align-middle">
                    <thead class="table-secondary">
                        <tr class="text-uppercase">
                            ...
                            <th scope="col" class="py-1">date posted</th>
                            <th scope="col" class="py-1">status</th>
                        </tr>
                    </thead>
                    <tbody>
                        @isset($user?->employer?->jobs)
                            @forelse ($user?->employer?->jobs as $j)
                                <tr class="">
                                    <td scope="row">
